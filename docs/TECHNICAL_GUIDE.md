...
地图相册是一个跨平台应用程序,支持桌面版(Windows、Mac、Linux)和 Web 版本。它的主要功能是让用户在世界地图上标记去过的地方,然后把照片和这些地点关联起来,形成一个可视化的旅行足迹记录。

桌面版使用 Electron 框架开发,简单来说就是用网页技术来做桌面软件。Web 版本可以直接在浏览器中运行,数据存储在 IndexedDB 中。界面部分用 React 写,地图用 Mapbox 渲染,桌面版数据存在本地的 SQLite 数据库里。

### 1.2 双版本架构

项目采用双版本架构,一套代码同时支持桌面和 Web 两种运行环境:

**桌面版 (Electron)**
- 数据存储: SQLite 数据库 (better-sqlite3)
- 图片处理: Sharp 库 (C++ 原生性能)
- 文件系统: 完整的本地文件访问能力
- 自动更新: electron-updater 支持

**Web 版本**
- 数据存储: IndexedDB (浏览器内置)
- 图片处理: Canvas API + createImageBitmap
- 文件系统: File API + Blob URL
- 离线支持: Service Worker + PWA

两个版本共享同一套 React 组件和业务逻辑,通过 API 层抽象屏蔽底层差异。
...
Zustand 是状态管理工具,用来在不同组件之间共享数据。比如当前选中的标记、地图的缩放级别这些信息都存在 Zustand 里。它比 Redux 简单很多,代码量少,学习成本低。项目定义了四个 Store:
- useAppStore: 应用全局状态(加载状态、Toast 消息等)
- useMapStore: 地图相关状态(标记列表、选中标记、地图设置等)
- useSearchStore: 搜索相关状态(搜索词、搜索结果、搜索历史等)
- useSettingsStore: 用户设置(主题、语言等)
...
│   └── utils/              # 工具函数
│       ├── helpers.js      # 通用工具函数
│       ├── propTypes.js    # PropTypes 验证器
│       ├── performanceProfiler.js  # 性能分析
│       ├── cdnManager.js   # CDN 管理
│       ├── pwaManager.js   # PWA 管理
│       ├── seoManager.js   # SEO 管理
│       ├── webAnalytics.js # Web 分析
│       └── autoUpdater.cjs # 自动更新
├── src-web/                # Web 版本入口
...
- Toast: 消息提示
- ErrorBoundary: 错误边界
- loaders/: 各种加载动画(MinimalLoader、ParticleMorphLoader 等)

**photo/ - 照片相关组件**
- PhotoViewer: 照片查看器
- PhotoEditor: 照片编辑器
- PhotoSearchPanel: 照片搜索面板
- LazyPhoto: 懒加载图片

**map/ - 地图相关组件**
- MarkerListItem: 标记列表项
...
**helpers.js - 通用工具函数 (30+)**
- 格式化函数: formatDate、formatFileSize、formatDuration
- 验证函数: isValidEmail、isValidUrl、isValidCoordinate
- 字符串处理: truncateText、capitalizeFirst、slugify
- 数组操作: uniqueBy、groupBy、chunk
- 对象操作: deepClone、pick、omit
- 异步工具: sleep、retry、debounce、throttle

**constants/index.js - 常量定义 (80+)**
- 地图常量: 默认中心点、缩放级别、瓦片配置
- 缓存常量: LRU 容量、过期时间
- UI 常量: 动画时长、尺寸限制
- 业务常量: 文件大小限制、支持的格式
...
- 继承: eslint:recommended、react/recommended、react-hooks/recommended
- 插件: jsx-a11y (无障碍检查)
- 自定义规则: 禁止 console.log、要求 PropTypes 等
...
- commit-msg: 检查提交信息格式 (Conventional Commits)

### 15.2 文档工具

**JSDoc 配置 (jsdoc.config.json)**
- 主题: clean-jsdoc-theme
- 输出: docs/api/
- 命令: npm run docs

**Storybook 配置 (.storybook/)**
- 框架: @storybook/react-vite
- 插件: essentials、a11y、interactions
...
**调试配置 (launch.json)**
- Electron Main: 调试主进程
- Electron Renderer: 调试渲染进程
- Chrome: 调试 Web 版本

**任务配置 (tasks.json)**
- 开发: npm run dev / npm run web:dev
- 构建: npm run build / npm run dist
- 测试: npm run test / npm run lint

## 第十六章 部署与发布

### 16.1 桌面版本发布

**GitHub Actions 自动构建 (build-release.yml)**
- 触发条件: 推送 v* 标签
- 构建平台: Windows (x64)、macOS (x64/arm64)、Linux (x64)
- 产物: NSIS 安装包、DMG、AppImage、deb、rpm

**自动更新 (autoUpdater.cjs)**
- 使用 electron-updater
- 检查更新: 启动时自动检查
- 下载更新: 后台下载,完成后提示
- 安装更新: 用户确认后重启安装
...
**CDN 配置 (cdnManager.js)**
- 静态资源: 自动添加 CDN 前缀
- 图片优化: WebP 格式、响应式尺寸

**SEO 优化 (seoManager.js)**
...
**发布流程**
1. 更新版本号: npm run version:bump
2. 生成更新日志: npm run version:generate
3. 创建标签: git tag v1.x.x
4. 推送触发构建: git push --tags
...
2024 年 12 月 21 日: 大规模代码重构和开发体验优化:
- 双版本架构: 支持桌面版和 Web 版,共享组件和业务逻辑
- 组件目录重组: 按功能模块分类 (ui/photo/map/layout/monitor)
- 状态管理优化: Zustand 四个 Store (app/map/search/settings)
- API 层抽象: 统一数据访问接口,屏蔽 Electron/Web 差异
- 常量统一管理: 80+ 业务常量,消除魔法数字
- 工具函数分类: 30+ 通用工具函数,完整 JSDoc 注释
- 代码质量工具: ESLint + Prettier + Husky + lint-staged
- 文档工具: JSDoc API 文档 + Storybook 组件文档
- VS Code 配置: tasks.json + launch.json + extensions.json
- Mock 数据: 开发环境模拟数据,支持 Mock API
- 部署配置: GitHub Actions 多平台构建 + Netlify 自动部署
- PWA 支持: Service Worker + manifest.json + 离线页面
- 自动更新: electron-updater 配置

2024 年 12 月 21 日: Globe 加载页面和 Web 版本优化:
- Globe 加载器: Mapbox Globe 投影地球样式,支持拖拽旋转
- 进度条动画: 加载完成后平滑变形为"进入地图"按钮
- flyTo 缩放动画: 点击进入后从地球视角缩放到用户位置
- 淡出过渡: 飞行动画完成后加载器淡出,无白屏闪烁
- Web 下载弹窗: 使用 createPortal 渲染到 body,避免样式冲突
- 下载按钮动画: 蓝紫渐变背景 + 脉冲发光效果
- Service Worker 优化: 开发环境禁用缓存,避免热更新问题
- WebAdapter 完善: 添加 addMarker、updateMarker、deleteMarker 等方法
